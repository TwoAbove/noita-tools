...
const subtextMap = {
	...
	'data/entities/animals/illusions/dark_alchemist.xml': ({ t }) => (
		<div>{t('$animal_dark_alchemist')}</div>
	),
		...
		>
			...
			<Modal.Body>
				{showSelected && selected.length > 0 && (
					<Row
						sm={5}
						className="p-3 justify-content-start align-items-baseline row-cols-auto"
					>
						{selected.map(entity => {
							return (
								<Col
									className="p-0 m-1 d-flex flex-column align-items-center text-center lh-1"
									key={entity}
								>
									<EntityView
										onClick={() =>
											handleSelectedClicked && handleSelectedClicked(entity)
										}
										id={entity}
									/>
								</Col>
				...
				<Row
					sm={5}
					className="p-3 justify-content-center align-items-baseline row-cols-auto"
				>
