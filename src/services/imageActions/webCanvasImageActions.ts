...
export const scaleImageData = (img: ImageData, s: number): HTMLCanvasElement => {
...
const g = (c: HTMLCanvasElement): Uint8ClampedArray => getContext(c).getImageData(0, 0, c.width, c.height)!.data;
...
export const iteratePixels = (image: ImageData, step: number, cb: (x: number, y: number, color: number) => void) => {
  let img: ImageData;
  if (!(image as any).data) {
    img = getPixels(image);
  } else {
...
export const getTilePos = (gx: number, gy: number) => {
  const x = Math.floor((gx - 5) / 512) + 35;
  let y = Math.floor((gy - 3) / 512) + 14;
  if (y === 14 && gy < 3) {
    y--;
  }
  return { x, y };
