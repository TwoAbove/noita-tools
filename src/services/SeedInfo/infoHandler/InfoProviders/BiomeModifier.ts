...
export class BiomeModifierInfoProvider extends InfoProvider {
	...
	availableBiomeModifiers = Object.values(biomeModifiersData as Objectify<
		typeof biomeModifiersData
	>).reduce(
		(c, bm) => {
	...
	has_modifiers(biome_name: string, ctx) {
		if (
			biome_name === 'coalmine' &&
			ctx.deaths < 8 &&
			ctx.should_be_fully_deterministic === false
		) {
			return false;
		}
	...
	provide() {
		...
		let biome_modifier_fog_of_war_clear_at_player =
			biome_modifiers['FOG_OF_WAR_CLEAR_AT_PLAYER'];
		let biome_modifier_cosmetic_freeze = biome_modifiers['FREEZING_COSMETIC'];

		const set_modifier_if_has_none = (
			biome_name: string,
			modifier_id: string
		) => {
		...
		for (let i = 0; i < biomes.length; i++) {
			...
			if (this.has_modifiers(biome_names[0], ctx)) {
				modifier = this.randoms.pick_random_from_table_weighted(
					rnd,
					Object.values(biome_modifiers)
				);
			}
    ...
    */
		...
		if (
			this.randoms.random_next(rnd, 0.0, 1.0) < this.CHANCE_OF_MOIST_FUNGICAVE
		) {
