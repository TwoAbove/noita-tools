...
export class ShopInfoProvider extends InfoProvider {
	...
	generate_shop_item(
		...
		let price = Math.max(
			Math.floor((spell.price * 0.3 + 70 * biomeid) / 10) * 10,
			10
		);
	...
	spawn_all_shop_items(
		...
		const numberOfExtraItems = [...pickedPerks.values()]
			.flat(2)
			.filter(p => p === 'EXTRA_SHOP_ITEM').length;
		const unshufflePerk = !![...pickedPerks.values()]
			.flat(2)
		...
		const type: IShopType =
			this.randoms.Random(0, 100) <= 50 ? IShopType.item : IShopType.wand;
		const items: any[] = [];
		for (let i = 0; i < count; i++) {
			if (type === IShopType.item) {
				// To simulate the position of items
				items[i] = this.generate_shop_item(
					x + i * item_width,
					y - 30,
					false,
					undefined,
				...
				items[i + count] = this.generate_shop_item(
					x + i * item_width,
					y,
					i === sale_item_i,
					undefined,
