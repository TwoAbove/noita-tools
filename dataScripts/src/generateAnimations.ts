...
(async () => {
  ...
  let files = await recursiveReaddir(path.resolve(noitaData, "data/entities"), ["*.png", "*.lua", "*.txt"]);
  ...
  const parseSpriteArray = async (breakdowns: FrameBreakdown[]): Promise<IVisualData> => {
  ...
  const parseEntity = async (entityData: any): Promise<any> => {
    ...
    } else if (entity.PhysicsImageShapeComponent && entity.PhysicsImageShapeComponent[0].image_file) {
