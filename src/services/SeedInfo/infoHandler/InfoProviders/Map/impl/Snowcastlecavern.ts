...
class Snowcastlecavern extends Base {
  chestLevel = 3;
  g_lamp = [
  ...
  spawn_hourglass_blood(x: number, y: number) {
    this.warn(`$ TODO: AUTO_GEN not implemented for ${this.constructor.name}`);
  }
  spawn_hourglass_master(x: number, y: number) {
    this.warn(`$ TODO: AUTO_GEN not implemented for ${this.constructor.name}`);
  }
  spawn_hourglass_music_trigger(x: number, y: number) {
    this.warn(`$ TODO: AUTO_GEN not implemented for ${this.constructor.name}`);
  }
}
export default Snowcastlecavern;
