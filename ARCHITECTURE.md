...
The wasm boundary is split by ownership. Small deterministic seed modules (`rng`, `alchemy`, `fungal`) live in `src/services/SeedInfo/wasm` as Zig entries with shared core code, and TypeScript loads their raw exports through `src/services/SeedInfo/wasm/compiled.ts`. The map/wang path in `src/services/SeedInfo/noita_random` keeps the C++ tiling code, but it now builds directly with Zig's C++ toolchain into a raw wasm module loaded by `src/services/SeedInfo/noita_random/mapWasm.ts`.
