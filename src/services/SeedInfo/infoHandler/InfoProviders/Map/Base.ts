...
export default class Base {
      ...
      spawns: [
        {
          ...
          load_entity_func: (data, x, y) => {
            ...
            if (this.GameHasFlagRun("greed_curse") && this.GameHasFlagRun("greed_curse_gone") === false) {
              this.EntityLoad("data/entities/items/pickup/physics_gold_orb_greed.xml", x + ox, y + oy);
            } else {
              this.EntityLoad("data/entities/items/pickup/physics_gold_orb.xml", x + ox, y + oy);
            }
          },
          offset_y: -2,
        },
        ...
        },
        {
          ...
          load_entity_func: (data, x, y) => {
            ...
            const opts = ["laser", "fireball", "lava", "slow", "null", "disc", "metal"];
        ...
        },
        {
          ...
          load_entity_func: (data, x, y) => {
            ...
            if (this.GameHasFlagRun("greed_curse") && this.GameHasFlagRun("greed_curse_gone") === false) {
              this.EntityLoad("data/entities/items/pickup/physics_greed_die.xml", x + ox, y + oy);
            } else {
              this.EntityLoad("data/entities/items/pickup/physics_die.xml", x + ox, y + oy);
            }
          },
          offset_y: -12,
  ...
  spawn(what: IProb[], x, y, rand_x?, rand_y?) {
    ...
    if (v) {
      this.entity_load_camera_bound(v, x + x_offset, y + y_offset, rand_x, rand_y);
    }
  }
  ...
  spawn_altar_top(x: number, y: number, is_solid: boolean) {
    ...
    if (is_solid) {
      this.LoadPixelScene("data/biome_impl/temple/solid.png", "", x, y - 40 + 300, "", true);
    }
  }
  ...
  load_random_pixel_scene(what: IProbScene[], x: number, y: number) {
    let r = new D(this.randoms.ProceduralRandomf(x, y, 0, 1) * this.total_prob(what, x));
      ...
      if (v.color_material) {
        ...
        for (let [color, material] of Object.entries(v.color_material)) {
          ...
          if (typeof material !== "string") {
            res = material[Math.ceil(this.randoms.ProceduralRandomf(x + 11, y - 21, 0, 1) * material.length) - 1];
          } else {
            res = material;
          }
    ...
    this.err("ERROR - director_helpers.lua - load_random_pixel_scene() shouldn't reach here");

    if (last_element) {
      if (!last_element.material_file && !last_element.visual_file && !last_element.background_file) {
        this.LoadPixelScene(last_element.material_file!, last_element.visual_file!, x, y);
      } else {
        this.err("ERROR " + what.length);
        this.err("ERROR - director_helpers.lua - load_random_pixel_scene() should it be loading a scene? ");
      }
    } else {
      this.err("ERROR " + what.length);
      this.err("ERROR - director_helpers.lua - load_random_pixel_scene() shouldn't reach here");
  ...
  entity_load_camera_bound(entity_data: IProb, x: number, y: number, rand_x = 4, rand_y = 4) {
    if (entity_data.entities) {
      entity_data.entities.forEach((ev, j) => {
        if (typeof ev === "object") {
          let count = 1;
          if (ev.min_count && ev.max_count) {
            count = this.randoms.ProceduralRandomf(x + j, y, ev.min_count, ev.max_count);
          }

          for (let i = 1; i <= count; i++) {
            let pos_x = x + this.randoms.ProceduralRandomf(x + j, y + i, -rand_x, rand_x);
            let pos_y = y + this.randoms.ProceduralRandomf(x + j, y + i, -rand_y, rand_y);
            if (ev.offset_y) {
          ...
          let pos_x = x + this.randoms.ProceduralRandomf(x + j, y, -rand_x, rand_x);
          let pos_y = y + this.randoms.ProceduralRandomf(x + j, y, -rand_y, rand_y);
          // if (ev.offset_y) {
          // 	pos_y = pos_y + ev.offset_y
          // }
          // if (ev.offset_x) {
    ...
    const how_many = this.randoms.ProceduralRandomf(x, y, entity_data.min_count, entity_data.max_count);
    if (how_many <= 0) {
      return;
    }
  ...
  spawn_heart(x: number, y: number) {
    ...
    if (r > heart_spawn_percent) {
      const entity = this.EntityLoad("data/entities/items/pickup/heart.xml", x, y);
    } else if (r > 0.3) {
      ...
      if (rnd <= 90 || y < 512 * 3) {
        ...
        if (rnd < 1000) {
          const entity = this.EntityLoad("data/entities/items/pickup/chest_random.xml", x, y);
        } else {
          const entity = this.EntityLoad("data/entities/items/pickup/chest_random_super.xml", x, y);
        }
      } else {
        ...
        if (rnd <= 95) {
          const entity = this.EntityLoad("data/entities/animals/chest_mimic.xml", x, y);
        } else {
          const entity = this.EntityLoad("data/entities/items/pickup/chest_leggy.xml", x, y);
        }
      }
    }
  }
  ...
  spawn_from_list(list: any, x: number, y: number) {
    ...
    for (const data of spawnlist.spawns) {
      ...
      if (rnd >= vmin && rnd <= vmax) {
        if (data.spawn_requires_flag && this.HasFlagPersistent(data.spawn_requires_flag) === false) {
          return;
        }
  ...
  spawn_ghostlamp(x: number, y: number) {
    if (!this.g_ghostlamp) {
      this.err(`g_ghostlamp needs to exist on class ${this.constructor.name}`);
      return;
    }
  ...
  spawn_potion_altar(x: number, y: number) {
    ...
    if (r > 0.65) {
      ...
      this.LoadPixelScene("data/biome_impl/potion_altar.png", "", x - 5, y - 15, "", true);
    }
  }
  spawn_collapse(x, y) {
  ...
  spawn_wand_trap_electricity_source(x, y) {
    this.EntityLoad("data/entities/buildings/wand_trap_electricity.xml", x, y);
  }
  ...
  spawn_apparition(x, y) {
    ...
    if (state === PlaceItems1 || state === PlaceItems2) {
      ...
      this.LoadPixelScene("data/biome_impl/grave.png", "data/biome_impl/grave_visual.png", x - 10, y - 15, "", true);
    }
    // --[[
