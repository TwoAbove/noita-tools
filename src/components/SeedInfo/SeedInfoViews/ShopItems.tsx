	...
	return (
		<div
			className={classNames(
				'm-2 text-center d-flex flex-column justify-content-center align-items-center',
				className
			)}
		>
			<Icon
...
const ItemShop = (props: {
	...
	return (
		<div>
			<Row className="p-3">
				These assume that you have all spells unlocked. Configuration coming
				soon!
			</Row>
				...
				{shop.items.map((item, i) => {
					...
					return (
						<Col key={i}>
							<Item
								highlight={highlight}
								className={classNames(highlight && 'mb-3')}
								item={item}
							/>
						</Col>
					);
								...
								<tr>
									...
									<td>
										<Icon
											size="2rem"
											uri={
												spellInfoProvider.provide(item.cards.permanentCard)
													.sprite
											}
											title={t(
												spellInfoProvider.provide(item.cards.permanentCard).name
											)}
											background
										/>
									</td>
...
const Shop = (props: IShopProps) => {
	...
	return (
		...
		>
			...
			<Modal.Body>
				{shop && <ShopItems shop={shop} isFavorite={isFavorite} />}
			</Modal.Body>
		</Modal>
	);
};
