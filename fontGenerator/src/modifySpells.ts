...
(async () => {
	{
		const data: any[] = JSON.parse(
			fs.readFileSync(path.resolve(root, 'spells.json')).toString()
		);
	...
	}
	{
		const data: { [key: string]: any } = JSON.parse(
			fs.readFileSync(path.resolve(root, 'obj/spells.json')).toString()
		);
    ...
    for (const d of out) {
      out2[d["id"]] = d;
		...
		fs.writeFileSync(
			path.resolve(root, 'obj/spells.json'),
			JSON.stringify(out2, null, 4)
		);
	}
})();
