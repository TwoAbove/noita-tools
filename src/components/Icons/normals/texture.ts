...
class Texture {
	...
	onload() {
		...
		this.textureData = this.renderContext.getImageData(
			0,
			0,
			width,
			height
	...
	moveLight(pos: ILightPosition) {
		...
		this.imageData =
			this.imageData || this.renderContext.getImageData(0, 0, width, height);
		...
		for (y = 0; y < height; y++) {
			for (x = 0; x < width; x++) {
				...
				dot =
					dx * normals[inorm++] + dy * normals[inorm++] + dz * normals[inorm++];
