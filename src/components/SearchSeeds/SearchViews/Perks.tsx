...
const Perks: FC<IPerksProps> = ({ onUpdateConfig, config }) => {
	...
	return (
		<Container fluid>
			<p>
				<b>Deck:</b> Choose which perks must be in the deck.{' '}
				<b>All of:</b> <i>All</i> perks must be in the Holy Mountain.{' '}
				<b>Some of:</b> At least <i>one</i> of the perks must be in the Holy Mountain.{' '}
				<br/>
					...
					<Row className='justify-content-start row-cols-auto'>
						{perksDeck[0].map(perkId => {
							return (
								...
								>
									<Perk
										onClick={() => handleDelete('deck', perkId, 0)}
										perk={perkInfoProvider.perks[perkId]}
									/>
								</Col>
							);
