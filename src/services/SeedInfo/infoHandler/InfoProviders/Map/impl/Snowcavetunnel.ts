...
class Snowcavetunnel extends Base {
  ...
  init(x: number, y: number, w, h) {
    for (let i = 0; i < 8; i++) {
      ...
      let pos_x = this.randoms.ProceduralRandomi(109, i * 53, biome_x_min, biome_x_max);
      let pos_y = this.randoms.ProceduralRandomi(111, i * 2.9, biome_y_min, biome_y_max);

      if (pos_x >= x && pos_x <= x + w && pos_y >= y && pos_y <= y + h) {
        this.LoadPixelScene("data/biome_impl/snowcave/statue_hand.png", "", pos_x - 22, pos_y - 22, "", true);
      }
    }
  }
  ...
  load_puzzle_capsule_b(x: number, y: number) {
    this.load_random_pixel_scene(this.g_puzzle_capsule_b, x - 50, y - 230);
  }
  ...
  spawn_burning_barrel(x: number, y: number) {
    if (this.safe(x, y)) {
      this.EntityLoad("data/entities/props/physics_barrel_burning.xml", x, y);
    }
  }
  ...
  spawn_buried_eye_teleporter(x: number, y: number) {
    this.EntityLoad("data/entities/buildings/teleport_snowcave_buried_eye.xml", x, y);
  }
