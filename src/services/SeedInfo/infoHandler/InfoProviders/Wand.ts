...
export class WandInfoProvider extends InfoProvider {
	...
	GetWandUI(gun: IGun): (typeof this.wands)[number] {
		...
		gun_in_wand_space.deck_capacity = clamp(
	...
	get_gun_data(
		...
		if (p < 20) {
			gun['mana_charge_speed'] =
				(50 * level + this.randoms.Random(-5, 5 * level)) / 5;
			gun['mana_max'] =
				(50 + 150 * level + this.randoms.Random(-5, 5) * 10) * 3;
		}
		...
		if (gun['reload_time'] >= 60) {
			...
			if (this.randoms.Random(0, 100) < 50) {
				...
				for (let i = 0; i < 6; i++) {
					let temp_actions_per_round = this.randoms.Random(
	...
	apply_random_variable(t_gun: IGun, variable: string): void {
		...
		if (variable === 'reload_time') {
			...
			t_gun[variable] = clamp(
				this.randoms.RandomDistribution(
					probs.min,
					probs.max,
					probs.mean,
					probs.sharpness
		...
		if (variable === 'speed_multiplier') {
			t_gun[variable] = this.randoms.RandomDistributionf(
				probs.min,
				probs.max,
				probs.mean,
				probs.sharpness
	...
	wand_add_random_cards(x: number, y: number, gun: IGun, level = 1): IGunCards {
		...
		let bullet_card = this.randoms.GetRandomActionWithType(
			x,
			y,
			level,
		...
		if (this.randoms.Random(0, 100) < 50) {
			...
			while (this.randoms.Random(1, 10) === 10) {
				...
				bullet_card = this.randoms.GetRandomActionWithType(
					x,
					y,
					extra_level,
					ACTION_TYPE.PROJECTILE,
