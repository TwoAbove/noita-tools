...
const Compute = () => {
	...
	return (
		<Container>
			<Row>
				<p>
					This tab allows you to use this device's computing power to help with searches.
				</p>
				<div className='ps-3'>
					<ul>
						<li>
							If you're doing your own searches on other devices, this device's compute will be used for that.
						</li>
						<li>
							If not, you can help other users with their searches and earn compute time for your own future searches.
						</li>
						<li>
							To see how much compute time you have, go your profile page.
						</li>
					</ul>
				</div>
			</Row>
			<hr />
			<Row><p>{connected ? 'Connected' : 'Not Connected'}</p> </Row>
			{/* <Row>
				<Col>
					<Form.Group>
			...
			<Stack gap={3}>
						...
						<Col md={8}>
							{computeInfo.jobName ?
								<Col>
									...
									<Col className='text-muted fw-light'>Cluster stats: Seeds checked: {localizeNumber(computeInfo.jobStats.checked)} (Estimated time left: {humanize(computeInfo.jobStats.estimate * 1000, { round: true, units: ["h", "m"] })}, {localizeNumber(Math.round(computeInfo.jobStats.rate * 100) / 100)} avg seeds/s)</Col>
				...
				<Row>
					...
					<Col>
						<Button disabled={computeRunning} onClick={() => handleStart()}>Start</Button>
					</Col>
					<Col>
						<Button disabled={!computeRunning} onClick={() => handleStop()}>Stop</Button>
					</Col>
				</Row>
			</Stack>
		</Container>
