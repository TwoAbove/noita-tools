...
export class MapInfoProvider extends InfoProvider {
	...
	async getMap(x: number, y: number, color) {
		const mapData = this.maps[color];
	...
	async getPathMap(x: number, y: number, color) {
		const area = this.getArea(x, y, color);
		const cacheKey = `${color} ${area.x1} ${area.y1}`;
		...
		const map = await this.getMap(x, y, color);
		if (!map) {
			return;
		}

		const pathMap = this.randoms.GetPathMap(map, map.width, map.height, x, y);
		const pathMapScaled = await scaleImageData(pathMap, 10);
	...
	async getInterestPoints(
		...
		const pathMap = await this.getPathMap(x, y, color);
		...
		const LoadPixelScene = async (
			...
			const image = this.imageCache.has(path)
				? this.imageCache.get(path)!
				: await imageFromBase64(impl.src);

			const { px, py } = getLocalPos(x, y, gx, gy);
			drawImageData(image, areaMap, px, py, color_to_material_table);
			await iteratePixels(image, 1, async (dx, dy, c) => {
				...
				await mapImplementation.handle(
					funcName,
					gx + dx,
					gy + dy,
					1,
		...
		try {
			...
			await iteratePixels(areaMap, 10, async (px, py, c) => {
				...
				await mapImplementation.handle(funcName, gx, gy, 10, 10, is_open_path);
			});
		} catch (e) {
	...
	async provide(
		...
		const startTime = performance.now();
		const areaMap = await this.getMap(tx, ty, color);
		if (!areaMap) {
			console.groupEnd();
			return;
		}
		...
		const interestPoints = await this.getInterestPoints(
			area.x1,
			area.y1,
			color,
			areaMapToScale
