...
class Snowcastlehourglasschamber extends Base {
  chestLevel = 3;
  g_items = [
    {
      prob: 1,
  ...
  init(x: number, y: number) {
    this.LoadPixelScene(
      "data/biome_impl/snowcastle/hourglass_chamber.png",
      "",
      x,
      y,
      "data/biome_impl/snowcastle/hourglass_chamber_background.png",
      true,
    );
  }
  ...
  spawn_teleporter(x: number, y: number) {
    this.EntityLoad("data/entities/buildings/teleport_hourglass_return.xml", x, y);
  }
  ...
  spawn_items(x: number, y: number) {
    ...
    // const spell = this.make_random_card(x, y);
    const spell = "CESSATION";
    this.HandleInterest("Spell", x, y, { spell });
  }
}
export default Snowcastlehourglasschamber;
