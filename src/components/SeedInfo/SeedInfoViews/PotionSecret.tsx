...
interface IPotionSecretProps {
  x: number;
  y: number;
}

const PotionSecret: FC<IPotionSecretProps> = ({ x, y }) => {
  const { gameInfoProvider } = useContext(GameInfoContext);

  const [t] = useTranslation("materials");

  const potion = useMemo(() => gameInfoProvider!.providers.potionSecret.provide(x, y), [gameInfoProvider, x, y]);
