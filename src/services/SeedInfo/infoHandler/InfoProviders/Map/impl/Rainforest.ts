...
class Rainforest extends Base {
  ...
  init(x: number, y: number, w: number, h: number) {
    ...
    const spawn_statue = (statue_num, spawn_x, spawn_y, ray_dir_x, ray_dir_y) => {
      let [_, x1, y1] = this.RaytracePlatforms(spawn_x, spawn_y, spawn_x + ray_dir_x * 100, spawn_y + ray_dir_y * 100);
      ...
      this.LoadPixelScene(
        ...
        "data/biome_impl/rainforest/rainforest_statue_0" + statue_num + "_visual.png",
        x1,
        y1,
        "",
        true
    ...
    let pos_x = this.randoms.ProceduralRandomi(87, -254, biome_x_min, biome_x_min + rim);
    ...
    // -- statue 1, right
    pos_x = this.randoms.ProceduralRandomi(8, -5, biome_x_max - rim, biome_x_max);
    if (is_inside_tile(pos_x, pos_y)) spawn_statue(1, pos_x, pos_y, 1, 0);

    // -- statue 2, top
    pos_x = portal_x - 13;
    pos_y = this.randoms.ProceduralRandomi(1999, 7, biome_y_min, biome_y_min + rim);
    if (is_inside_tile(pos_x, pos_y)) spawn_statue(2, pos_x, pos_y, 0, 1);
    // -- statue 2, bottom
    pos_y = this.randoms.ProceduralRandomi(-415, 40, biome_y_max - rim, biome_y_max);
    if (is_inside_tile(pos_x, pos_y)) spawn_statue(2, pos_x, pos_y, 0, 1);
  }
