...
export class WeatherInfoProvider extends InfoProvider {
	provide() {
		...
		const snows =
			(snows1 || snows2) &&
			this.randoms.random_next(rnd_time, 0.0, 1.0) <= snowfall_chance; // snow is based on real world time
		const rains =
		...
		if (weather.rain_type === RAIN_TYPE_NONE) {
			...
			weather.clouds = Math.max(
				weather.fog,
				this.randoms.random_next(rnd, 0.0, 1.0)
			);
			weather.rain_draw_long =
				this.randoms.random_next(rnd, 0.0, 1.0) <=
				weather.rain_draw_long_chance!;
			weather.rain_particles = this.randoms.random_next(
				rnd,
				weather.rain_particles_min!,
				weather.rain_particles_max!
			);
		}
