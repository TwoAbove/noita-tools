...
class Coalmine extends Base {
  ...
  g_oiltank = [
    {
      ...
      is_unique: 0,
      color_material: {
        f0bbeeff: ["water", "oil", "water", "oil", "alcohol", "sand", "coal", "radioactive_liquid"],
      },
    },
    {
      prob: 0.0004,
    ...
    },
    {
      ...
      is_unique: 0,
      color_material: {
        f0bbeeff: ["water", "oil", "water", "oil", "alcohol", "oil", "coal", "radioactive_liquid"],
      },
    },
    {
      prob: 1,
    ...
    },
    {
      ...
      is_unique: 0,
      color_material: {
        f0bbeeff: ["water", "oil", "water", "oil", "alcohol", "radioactive_liquid", "coal", "radioactive_liquid"],
      },
    },
    {
      prob: 0.05,
  ...
  spawn_small_enemies(x: number, y: number, w?, h?, is_open_path?) {
    if (is_open_path) {
      ...
      this.spawn_with_limited_random(this.g_small_enemies, x, y, 0, 0, ["longleg", "fungus"]);
    } else {
      this.spawn_with_limited_random(this.g_small_enemies, x, y, 0, 0, ["longleg", "fungus"]);
    }
  }
  ...
  spawn_big_enemies(x: number, y: number, w?, h?, is_open_path?) {
    if (is_open_path) {
      ...
      this.spawn_with_limited_random(this.g_big_enemies, x, y, 0, 0, ["longleg", "fungus"]);
    } else {
      this.spawn_with_limited_random(this.g_big_enemies, x, y, 0, 0, ["longleg", "fungus"]);
    }
  }
  ...
  load_altar(x: number, y: number) {
    this.LoadPixelScene("data/biome_impl/altar.png", "data/biome_impl/altar_visual.png", x - 92, y - 96, "", true);
  ...
  spawn_chest(x: number, y: number) {
    ...
    if (rnd >= super_chest_spawn_rate - 1) {
      this.EntityLoad("data/entities/items/pickup/chest_random_super.xml", x, y);
    } else {
      this.EntityLoad("data/entities/items/pickup/chest_random.xml", x, y);
    }
  }
  ...
  spawn_bbqbox(x: number, y: number) {
    ...
    if (rnd <= 99) {
      ...
      this.EntityLoad("data/entities/items/pickup/jar_of_urine.xml", x, y);
      this.EntityLoad("data/entities/animals/shotgunner_weak.xml", x + 10, y);
    }
  }

  spawn_swing_puzzle_box(x: number, y: number) {
    this.EntityLoad("data/entities/props/physics/trap_electricity_suspended.xml", x, y);
  }

  spawn_swing_puzzle_target(x: number, y: number) {
    this.EntityLoad("data/entities/buildings/swing_puzzle_target.xml", x, y);
  }
  ...
  spawn_receptacle_oil(x: number, y: number) {
    ...
    this.EntityLoad("data/entities/items/pickup/potion_empty.xml", x + 72, y - 17);
  }
}
