...
const questionMark = 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAYAAAAGCAIAAABvrngfAAAABnRSTlMAAAAAAABupgeRAAAAJ0lEQVR4nGNkYGB4O+EhAwMDAwODcIE8AzIfjY1N6O2Eh1iU4NMCAGxKETbGzzNZAAAAAElFTkSuQmCC';
...
const NormalMapRenderer = ({ material, animations, ...rest }: INormalMapRendererProps) => {
	...
	useEffect(() => {
		(() => {
			NoitaTexture(color, animations.actions[animations.default].src[0]).then(texture => {
				setTexture(texture);
			}).catch(e => {
				console.error(e);
			});
...
const MemoizedNormalMapRenderer = memo(NormalMapRenderer, (p, n) => p.material === n.material && p.animations === n.animations);
...
const WandModal: FC<IWandModalProps> = ({ x, y, cost, level, force_unshuffle }) => {
	...
	const wand = useMemo(() => gameInfoProvider!.providers.wand.provide(x, y, cost, level, !!force_unshuffle, !!unshufflePerk), [gameInfoProvider, x, y, cost, level, force_unshuffle, unshufflePerk]);
	const favoriteSpells = ([wand.cards.permanentCard, ...wand.cards.cards].filter(Boolean) as string[]).filter(id => isSpellFavorite(id));
...
const Entity: FC<EntityProps> = ({ id, action, entityParams = {}, preview = false, ...rest }) => {
	...
	if (id.startsWith('data/entities/items/wand_')) {
		...
		const { x, y, extra: { cost, level, force_unshuffle } } = entityParams;
