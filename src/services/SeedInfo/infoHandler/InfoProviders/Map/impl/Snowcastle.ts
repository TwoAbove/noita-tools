...
class Snowcastle extends Base {
  ...
  g_pods_large = [
    ...
    },
    {
      ...
      visual_file: '',
      background_file:
        'data/biome_impl/snowcastle/pod_large_01_background_b.png',
      is_unique: 0
    },
    {
      prob: 1,
      ...
      visual_file: '',
      background_file:
        'data/biome_impl/snowcastle/pod_large_01_background_c.png',
      is_unique: 0
    }
  ];
  g_pods_small_l = [
    ...
    },
    {
      ...
      visual_file: '',
      background_file:
        'data/biome_impl/snowcastle/pod_small_l_01_background.png',
      is_unique: 0
    },
    {
      prob: 1,
      ...
      visual_file: '',
      background_file:
        'data/biome_impl/snowcastle/pod_small_l_01_background_b.png',
      is_unique: 0
    }
  ];
  g_pods_small_r = [
    ...
    },
    {
      ...
      visual_file: '',
      background_file:
        'data/biome_impl/snowcastle/pod_small_r_01_background.png',
      is_unique: 0
    },
    {
      prob: 1,
      ...
      visual_file: '',
      background_file:
        'data/biome_impl/snowcastle/pod_small_r_01_background_b.png',
      is_unique: 0
    }
  ];
  g_furniture = [
  ...
  async spawn_turret(x: number, y: number) {
    if (this.safe(x, y)) {
      await this.spawn(this.g_turret, x, y, 0, 0);
    }
  }
  ...
  async spawn_lamp(x: number, y: number) {
    if (this.safe(x, y)) {
      await this.spawn(this.g_lamp, x + 5, y, 0, 0);
    }
  }
  ...
  async load_panel_01(x: number, y: number) {
    await this.load_paneling(x - 15, y - 30, '01');
  }
  async load_panel_02(x: number, y: number) {
    await this.load_paneling(x - 10, y - 20, '02');
  }
  async load_panel_03(x: number, y: number) {
    await this.load_paneling(x - 60, y - 20, '03');
  }
  async load_panel_04(x: number, y: number) {
    await this.load_paneling(x - 20, y - 20, '04');
  }
  async load_panel_05(x: number, y: number) {
    await this.load_paneling(x - 60, y - 60, '05');
  }
  async load_panel_06(x: number, y: number) {
    await this.load_paneling(x - 20, y - 60, '06');
  }
  async load_panel_07(x: number, y: number) {
    await this.load_paneling(x - 40, y - 40, '07');
  }
  async load_panel_08(x: number, y: number) {
    await this.load_paneling(x - 40, y - 20, '08');
  }
  async load_panel_09(x: number, y: number) {
    await this.load_paneling(x - 20, y - 20, '09');
  }
  ...
  async spawn_barricade(x: number, y: number) {
    await this.spawn(this.g_barricade, x, y, 0, 0);
  }
  async spawn_forcefield_generator(x: number, y: number) {
    if (!this.safe(x, y)) {
      return;
    }
    await this.spawn(this.g_forcefield_generator, x, y - 2, 0, 0);
  }
  async spawn_brimstone(x: number, y: number) {
    await this.EntityLoad('data/entities/items/pickup/brimstone.xml', x, y);
    await this.EntityLoad(
      'data/entities/buildings/sauna_stove_heat.xml',
      x,
      y + 10
    );
  }
  async spawn_vasta_or_vihta(x: number, y: number) {
    if (x > 190) {
      await this.EntityLoad('data/entities/items/wand_vasta.xml', x, y);
    } else {
      await this.EntityLoad('data/entities/items/wand_vihta.xml', x, y);
    }
  }

  async load_chamfer_top_r(x: number, y: number) {
    if (!this.safe(x, y)) {
      return;
    }
    await this.LoadPixelScene(
  ...
  async load_chamfer_top_l(x: number, y: number) {
    if (!this.safe(x, y)) {
      return;
    }
    await this.LoadPixelScene(
      'data/biome_impl/snowcastle/chamfer_top_l.png',
      '',
      x - 1,
      y,
      '',
  ...
  async load_chamfer_bottom_r(x: number, y: number) {
    if (!this.safe(x, y)) {
      return;
    }
    await this.LoadPixelScene(
      'data/biome_impl/snowcastle/chamfer_bottom_r.png',
      '',
      x - 10,
      y - 20,
      '',
  ...
  async load_chamfer_bottom_l(x: number, y: number) {
    if (!this.safe(x, y)) {
      return;
    }
    await this.LoadPixelScene(
      'data/biome_impl/snowcastle/chamfer_bottom_l.png',
      '',
      x - 1,
      y - 20,
      '',
  ...
  async load_chamfer_inner_top_r(x: number, y: number) {
    if (!this.safe(x, y)) {
      return;
    }
    await this.LoadPixelScene(
      'data/biome_impl/snowcastle/chamfer_inner_top_r.png',
      '',
      x - 10,
      y,
      '',
  ...
  async load_chamfer_inner_top_l(x: number, y: number) {
    if (!this.safe(x, y)) {
      return;
    }
    await this.LoadPixelScene(
      'data/biome_impl/snowcastle/chamfer_inner_top_l.png',
      '',
      x,
      y,
      '',
  ...
  async load_chamfer_inner_bottom_r(x: number, y: number) {
    if (!this.safe(x, y)) {
      return;
    }
    await this.LoadPixelScene(
  ...
  async load_chamfer_inner_bottom_l(x: number, y: number) {
    if (!this.safe(x, y)) {
      return;
    }
    await this.LoadPixelScene(
      'data/biome_impl/snowcastle/chamfer_inner_bottom_l.png',
      '',
      x,
      y - 20,
      '',
  ...
  async load_pillar_filler(x: number, y: number) {
    if (!this.safe(x, y)) {
      return;
    }
    await this.LoadPixelScene(
  ...
  async load_pillar_filler_tall(x: number, y: number) {
    if (!this.safe(x, y)) {
      return;
    }
    await this.LoadPixelScene(
      'data/biome_impl/snowcastle/pillar_filler_tall_01.png',
      '',
      x,
      y,
      '',
  ...
  async load_pod_large(x: number, y: number) {
    if (!this.safe(x, y - 50)) {
      return;
    }
    await this.load_random_pixel_scene(this.g_pods_large, x, y - 50);
  }
  async load_pod_small_l(x: number, y: number) {
    if (!this.safe(x, y - 40)) {
      return;
    }
    await this.load_random_pixel_scene(this.g_pods_small_l, x - 30, y - 40);
  }
  async load_pod_small_r(x: number, y: number) {
    if (!this.safe(x, y - 40)) {
      return;
    }
    await this.load_random_pixel_scene(this.g_pods_small_r, x - 10, y - 40);
  }
  async load_bunk_with_surprise(x: number, y: number) {
    await this.EntityLoad('data/entities/props/furniture_bunk.xml', x, y + 5);
    await this.EntityLoad('data/entities/props/physics_propane_tank.xml', x, y);
  }
  async load_furniture(x: number, y: number) {
    if (this.randoms.ProceduralRandomf(x, y, 0, 1) < 0.002) {
      await this.load_bunk_with_surprise(x, y);
    } else {
      await this.spawn(this.g_furniture, x, y + 5, 0, 0);
    }
  }
  async load_furniture_bunk(x: number, y: number) {
  ...
  async spawn_root_grower(x: number, y: number) {
    await this.EntityLoad('data/entities/props/root_grower.xml', x, y);
  }
  async spawn_forge_check(x: number, y: number) {
    await this.EntityLoad('data/entities/buildings/forge_item_check.xml', x, y);
  }
  async spawn_drill_laser(x: number, y: number) {
    await this.EntityLoad('data/entities/buildings/drill_laser.xml', x, y);
  }
  async spawn_cook(x: number, y: number) {
    await this.EntityLoad('data/entities/animals/miner_chef.xml', x, y);
  }
}
