...
const PerkSelect = (props: IPerkSelectProps) => {
  ...
  const perksToShow = fuse.search(filter || ' ').map(p => p.item).filter(p => !selectedPerks.includes(p.id));
          ...
          {perksToShow.map(perk => {
            return (
              <Col className="p-0 m-1" key={perk.id}>
                <Icon onClick={() => handleOnClick(perk.id)} uri={"data:image/png;base64," + perk.ui_icon} />
              </Col>
            );
          })}
        </Row>
