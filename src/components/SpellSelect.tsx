...
const SpellSelect: FC<ISpellSelectProps> = ({
  ...
  const spellsToShow = (filter ? fuse.search(filter).map(s => s.item) : shopInfoProvider.spellsArr)
    .filter(s =>
      level && level >= 0
        ? Object.keys(s.spawn_probabilities).includes(String(shopInfoProvider.getShopLevel(level)))
        : true
    )
