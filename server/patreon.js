...
const gatherMeData = (user, patreonUser) => {
	...
	let data = {
		...
		computeLeft:
			user.compute.patreonComputeLeft + user.compute.providedComputeLeft
	};

	const patronData = patronMembersCache.find(
		p => p.relationships.user.data.id === userId && p.type === 'member'
	);
...
const updatePatreonCompute = async () => {
	...
	try {
		...
		for (const user of users) {
			const patron = patronMembersCache.find(
				m => m.relationships.user.data.id === user.patreonId
			);
			let tierId;
			if (patron) {
				const tierIds = patron.relationships.currently_entitled_tiers.data.map(
